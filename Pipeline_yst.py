...
class pipeline_tstacks(object):
    ...
    def sampling(self, nsamples, verbose = True):
        ...
        '''
        sample_stack = tf.imread(self.fname, key = nsamples)
#        sample_stack = []
#        for ns in nsamples:
#            self.im.seek(ns)
...
#                    substack.append(np.array(self.im))
                substack = tf.imread(self.fname, key = np.arange(si, sf).astype('uint16'))
        ...
        fig_display  = plt.figure(figsize = (12,5))
        ax_1 = fig_display.add_subplot(121)
        ax_1.imshow(sample_frame, cmap = 'Greys_r')
        ax_1.axis('off')
        ax_1.set_title('Original frame')
        sample_frame = frame_deblur(sample_frame, sig = 5.0, Nit = 21)
        ax_2 = fig_display.add_subplot(122)
        ax_2.imshow(sample_frame, cmap = 'Greys_r')
        ax_2.scatter(cblobs[:,1], cblobs[:,0], s = 7, color = 'g')
        ax_2.axis('off')
        ax_2.set_title('# of blobs: '+ str(cblobs.shape[0]), fontsize = 16)
