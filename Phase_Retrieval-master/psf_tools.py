...
def psf_zplane(stack, dz, w0, de = 1):
    ...
    p0 = (a,0,w0,b)
    f = interp1d(zz, im_z, kind = 'cubic')
    zint = np.linspace(-zrange, zrange, 2*nz)
    im_interp = f(zint)
    plt.plot(zint, im_interp)
    plt.savefig('zprofile')
    
    try:
        #popt = optimize.curve_fit(gaussian, zz, im_z, p0)[0]
        popt = optimize.curve_fit(gaussian, zint, im_interp, p0)[0]
        z_offset = popt[1] # The original version is wrong
    except RuntimeError:
        print("Fitting fails.")
        
        ind_max = np.argmax(im_interp)
        z_offset = zint[ind_max]
    return z_offset, zz
